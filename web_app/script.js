...
async function getUsers() {
	...
	for (var i = 0; i < function_calls.length; i++) {
		...
		var amount = await BlockchainSplitwise.lookup(debtor, creditor);
...
async function getTotalOwed(user) {
	...
	var result = 0;
	for(var creditor of users){
		if(user !== creditor){
			result += await BlockchainSplitwise.lookup(user, creditor)
		}
	}
	...
	while (curBlock !== GENESIS) {
	  ...
	  for (var j = 0; j < txns.length; j++) {
		...
		if(txn.to == null){continue;}
		  ...
		  // Check that the function getting called in this txn is 'add_IOU'
		  if (func_call && func_call.name === "add_IOU" ) {
			var timeBlock = await provider.getBlock(curBlock);
			var args = func_call.params.map(function (x) {return x.value});
			if (
			  args[0].toLowerCase() === user.toLowerCase() ||
...
async function add_IOU(creditor, amount) {
	...
	var BlockchainSplitwiseWithSigner = BlockchainSplitwise.connect(provider.getSigner(defaultAccount))
	await BlockchainSplitwiseWithSigner.add_IOU(creditor, amount, path, min);
}
